class ByteMaker
{
    ...
    public ByteMaker(bool isCompressed)
    {
        ...
        _bitIndex = 4; // first bit indicates compressed, next 3 bits reserved to give data about trailing zeros
        _trailingZeros = 0;
    ...
    public void Add(char character)
    {
        ...
        for (int i = 7; i >= 0; i--)
        {
            int bit = (characterByte >> i) & 1; // right-shift, then AND with 1 to get "last" bit
    ...
    public byte[] Export()
    {
        ...
        _bytes[0] |= (byte)(_trailingZeros << 4); // push to bits 1-4, OR since already 0
    ...
    private void updateByte()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            newByte |= (byte)(_bits[i] << (7-i)); // left-shift, then OR with existing byte
        }
