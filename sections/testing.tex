...
\begin{longtable}{|c|p{0.45\linewidth}|p{0.41\linewidth}|}
    ...
    \hline
    2 & To ensure that the client can send a file to the server, and that the server can receive and store it, and to test again that the interactive mode on the client works & All is successful, the client can interactively send a file to the server, and the server can receive and store it properly \\
    \hline
    ...
    \hline
    11 & To ensure that plaintext files have their sizes significantly reduced by compression, and to verify that other files do not have their sizes increased & The compression works as expected, being very effective in compressing plaintext files, and not increasing the sizes of the other files \\
    \hline
