...
\begin{enumerate}
    ...
    \item I verify that the server has received a compressed version of this file, stored with a .huff extension to indicate this. The file size of the compressed version on the server is checked using the command \textit{du -sh [filename]}, and then compared to the same command run on the original file on the client device.
...
\hline
11 & 6a & Send 5 files from the client to the server: two regular plaintext files, a plaintext source code file, a png image file, and an mp4 video file & The three plaintext files should have their file size significantly reduced, and the other files should not have their file size increased by more than a byte & The first plaintext files had its size reduced from 60kB to 32kB, the second from 160kB to 88kB, and the C\# source code file from 16kB to 8kB. The other files did not appear to have their size changed (2.3MB and 1.5MB respectively before and after transmission). The increase of 1 byte associated with the compression will have been lost to rounding. & 1:04 - 2:10, \newline\newline 2:33 - 3:40 & Pass \\
\hline
...
\begin{longtable}{|c|p{0.45\linewidth}|p{0.41\linewidth}|}
    ...
    \hline
    3 & To ensure that the program can be run as one executable binary (rather than having to install dotnet and run the project with \textit{dotnet run}) & It can be run through one executable \\
    \hline
...
The test was successful, with all 500 test cases passing. Listing \ref{PrimalityTestOutput} provides a sample of the output.
